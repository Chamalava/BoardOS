  ...
  if (strcmp_P(cmd, PSTR("pinmode")) == 0) {
    ...
    if (strcmp_P(pinStr, PSTR("vixa")) == 0) {
      ...
      for (cycle = 0; cycle < count; cycle++) {
        size_t pinIndex;
        for (pinIndex = 0; pinIndex < demoPinCount; pinIndex++) {
          pinMode(demoPins[pinIndex], OUTPUT);
          digitalWrite(demoPins[pinIndex], HIGH);
          delay(50);
          digitalWrite(demoPins[pinIndex], LOW);
        }
      }
    ...
    Serial.println(BOARD_ARCH);
    Serial.print(F("Hardware: "));
    Serial.println(BOARD_HARDWARE);
    Serial.print(F("RAM: "));
    Serial.print(freeMemory());
